...
<button onClick={() => downloadAnswerFile(item._id)} className="flex items-center">
  ...
  <img src={download} alt="Download" />
    ...
    <div className="flex flex-row lg:gap-5 "> {/* Add this container for horizontal layout */}
      {answerFileChosen ? (
        ...
        <>
          <input
            ...
            style={{ display: "none" }}
          />
          <label htmlFor="answerFileInput" className='w-[8rem] flex flex-col justify-center'>
            <button
            ...
            >
              <p className='w-full flex flex-col justify-center mt-[1.5px]'>Choose File</p>
            </button>
          </label> 
        </>
      )}
...
<div className="flex items-center ">
  <button onClick={() => downloadFile(item._id, item.fileName)} className="flex items-center">
